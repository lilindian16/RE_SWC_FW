...
/* R8_USB_INT_FG */
#define USBFS_U_IS_NAK                                                         \
  0x80 // RO, indicate current USB transfer is NAK received for USB device mode
...
#define USBFS_UIF_FIFO_OV                                                      \
  0x10 // FIFO overflow interrupt flag for USB, direct bit address clear or
       // write 1 to clear
#define USBFS_UIF_HST_SOF                                                      \
  0x08 // host SOF timer interrupt flag for USB host, direct bit address clear
       // or write 1 to clear
#define USBFS_UIF_SUSPEND                                                      \
  0x04 // USB suspend or resume event interrupt flag, direct bit address clear
       // or write 1 to clear
#define USBFS_UIF_TRANSFER                                                     \
  0x02 // USB transfer completion interrupt flag, direct bit address clear or
       // write 1 to clear
#define USBFS_UIF_DETECT                                                       \
  0x01 // device detected event interrupt flag for USB host mode, direct bit
       // address clear or write 1 to clear
#define USBFS_UIF_BUS_RST                                                      \
  0x01 // bus reset event interrupt flag for USB device mode, direct bit address
       // clear or write 1 to clear

/* R8_USB_INT_ST */
#define USBFS_SETUP_ACT                                                        \
  0x80 // RO, indicate current USB transfer SETUP is complete
#define USBFS_UIS_TOG_OK 0x40 // RO, indicate current USB transfer toggle is OK
#define USBFS_UIS_TOKEN_MASK                                                   \
  0x30 // RO, bit mask of current token PID code received for USB device mode
...
#define USBFS_UIS_ENDP_MASK                                                    \
  0x0F // RO, bit mask of current transfer endpoint number for USB device mode
#define USBFS_UIS_H_RES_MASK                                                   \
  0x0F // RO, bit mask of current transfer handshake response for USB host mode:
       // 0000=no response, time out from device, others=handshake response PID
       // received

/* R8_UDEV_CTRL */
#define USBFS_UD_PD_DIS                                                        \
...
/* R8_UEPn_TX_CTRL */
#define USBFS_UEP_T_AUTO_TOG                                                   \
  (1 << 4) // enable automatic toggle after successful transfer completion on
           // endpoint 1/2/3: 0=manual toggle, 1=automatic toggle
#define USBFS_UEP_T_TOG                                                        \
  (1 << 6) // prepared data toggle flag of USB endpoint X transmittal (IN):
           // 0=DATA0, 1=DATA1
#define USBFS_UEP_T_RES_MASK                                                   \
  (3 << 0) // bit mask of handshake response type for USB endpoint X transmittal
           // (IN)
...
#define USBFS_UEP_R_AUTO_TOG                                                   \
  (1 << 4) // enable automatic toggle after successful transfer completion on
           // endpoint 1/2/3: 0=manual toggle, 1=automatic toggle
#define USBFS_UEP_R_TOG                                                        \
  (1 << 7) // expected data toggle flag of USB endpoint X receiving (OUT):
           // 0=DATA0, 1=DATA1
#define USBFS_UEP_R_RES_MASK                                                   \
  (3 << 2) // bit mask of handshake response type for USB endpoint X receiving
           // (OUT)
...
// RB_UEP_R_RES1 & RB_UEP_R_RES0: handshake response type for USB endpoint X
// receiving (OUT)
//   00: ACK (ready)
...
#define USBFS_UH_LOW_SPEED                                                     \
  0x04 // enable USB port low speed: 0=full speed, 1=low speed
#define USBFS_UH_BUS_RESET                                                     \
  0x02 // control USB bus reset: 0=normal, 1=force bus reset
#define USBFS_UH_PORT_EN                                                       \
  0x01 // enable USB port: 0=disable, 1=enable port, automatic disabled if USB
       // device detached
...
#define USBFS_UH_ENDP_MASK                                                     \
  0x0F // bit mask of endpoint number for USB host transfer

/* R8_UH_RX_CTRL */
#define USBFS_UH_R_AUTO_TOG                                                    \
  0x10 // enable automatic toggle after successful transfer completion: 0=manual
       // toggle, 1=automatic toggle
#define USBFS_UH_R_TOG                                                         \
  (1 << 7) // expected data toggle flag of host receiving (IN): 0=DATA0, 1=DATA1
#define USBFS_UH_R_RES                                                         \
  0x01 // prepared handshake response type for host receiving (IN): 0=ACK
       // (ready), 1=no response, time out to device, for isochronous
       // transactions

/* R8_UH_TX_CTRL */
#define USBFS_UH_T_AUTO_TOG                                                    \
  0x10 // enable automatic toggle after successful transfer completion: 0=manual
       // toggle, 1=automatic toggle
#define USBFS_UH_T_TOG                                                         \
  (1 << 6) // prepared data toggle flag of host transmittal (SETUP/OUT):
           // 0=DATA0, 1=DATA1
#define USBFS_UH_T_RES                                                         \
