...
volatile bool endpoint_rx_pending[DEF_UEP_NUM] = {false};

#define DEF_USBD_UEP0_SIZE  64
#define DEF_USB_EP1_FS_SIZE 64     // Endpoint 1 is used for HID report
#define DEF_USB_EP2_FS_SIZE 64 * 2 // Endpoint 2 is used for TX and RX comms

Descriptor_Configs_t *descriptor_config_ptr;
USB_Device_Mode_t     current_device_mode;

/**
 * Endpoint configuration:
 * Endpoint 0: config (required)
 * Endpoint 1: USB HID SWC mode (HID report)
 * Endpoint 2: bootloader comms (tx and rx)
 */
...
__attribute__((aligned(4))) uint8_t USBFS_EP3_Buf[DEF_USB_EP2_FS_SIZE];
__attribute__((aligned(4))) uint8_t USBFS_EP4_Buf[DEF_USB_EP2_FS_SIZE];
__attribute__((aligned(4))) uint8_t USBFS_EP5_Buf[DEF_USB_EP2_FS_SIZE];
__attribute__((aligned(4))) uint8_t USBFS_EP6_Buf[DEF_USB_EP2_FS_SIZE];
__attribute__((aligned(4))) uint8_t USBFS_EP7_Buf[DEF_USB_EP2_FS_SIZE];
...
void USBFS_Device_Endp_Init(void) {
  ...
  /* Config endpoint 1 (USB HID mode) */
  ...
  /* Config endpoint 2 (bootloader TX RX) */
...
void USBFS_Device_Init(FunctionalState       sta,
  ...
  descriptor_config_ptr = descriptor_config;
  current_device_mode   = device_mode;
  if (sta) {
    NVIC_DisableIRQ(USBFS_IRQn); // Make sure interrupt is disabled
    GPIO_USB_INIT();
    ...
    USBFSD->BASE_CTRL =
        USBFS_UC_DEV_PU_EN | USBFS_UC_INT_BUSY | USBFS_UC_DMA_EN;
    USBFSD->INT_FG    = 0xff;
    ...
    delayMicroseconds(10); // Don't use WCH delay implementation as that
                           // resets Arduino millis
    USBFSD->BASE_CTRL = 0x00;
...
void MCU_Sleep_Wakeup_Operate(void) {
  ...
  if (EXTI_GetFlagStatus(EXTI_Line12 | EXTI_Line13 | EXTI_Line14 |
                         EXTI_Line15) != RESET) {
    EXTI_ClearFlag(EXTI_Line12 | EXTI_Line13 | EXTI_Line14 | EXTI_Line15);
    USBFS_Send_Resume();
        ...
        if (buf_load_offset == 0) {
          if (mod == DEF_UEP_DMA_LOAD) {
            ...
            *uep_dma = (uint16_t)(uint32_t)pbuf;
            ...
            /* copy mode */
            memcpy(
                ((uint8_t *)(*((volatile uint32_t *)(uep_dma))) + 0x20000000),
                pbuf, len);
          }
        } else {
          memcpy(((uint8_t *)(*((volatile uint32_t *)(uep_dma))) + 0x20000000) +
                     buf_load_offset,
                 pbuf, len);
        }
        /* tx length */
...
void USBFS_IRQHandler(void) {
  ...
  case USB_DEVICE_MODE_BOOTLOADER:
    if (intflag & USBFS_UIF_TRANSFER) {
        ...
        case USBFS_UIS_TOKEN_IN | DEF_UEP0:
          ...
          if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) != USB_REQ_TYP_STANDARD) {
            ...
            case USB_GET_DESCRIPTOR:
              len = USBFS_SetupReqLen >= DEF_USBD_UEP0_SIZE ? DEF_USBD_UEP0_SIZE
                                                            : USBFS_SetupReqLen;
      ...
      case USBFS_UIS_TOKEN_OUT:
        ...
        case USBFS_UIS_TOKEN_OUT | DEF_UEP0:
          ...
          if (intst & USBFS_UIS_TOG_OK) {
            if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) !=
        ...
        case (USBFS_UIS_TOKEN_OUT | DEF_UEP2):
          len = USBFSD->RX_LEN;
          if (len <= sizeof(USBFS_EP2_Buf) / 2) {
            endpoint_rx_pending[2] = true;
      ...
      case USBFS_UIS_TOKEN_SETUP:
        ...
        if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) != USB_REQ_TYP_STANDARD) {
          ...
          case USB_GET_DESCRIPTOR:
            ...
            case USB_DESCR_TYP_DEVICE:
              pUSBFS_Descr = descriptor_config_ptr->device_descriptor;
              len          = descriptor_config_ptr->device_descriptor[0];
              break;

            /* get usb configuration descriptor */
            case USB_DESCR_TYP_CONFIG:
              pUSBFS_Descr = descriptor_config_ptr->config_descriptor;
              len = ((uint16_t)descriptor_config_ptr->config_descriptor[2] +
                     ((uint16_t)descriptor_config_ptr->config_descriptor[3]
                      << 8));
              break;
            ...
            case USB_DESCR_TYP_STRING:
              ...
              case DEF_STRING_DESC_LANG:
                pUSBFS_Descr = descriptor_config_ptr->language_descriptor;
                len          = descriptor_config_ptr->language_descriptor[0];
                break;

              /* Descriptor 1, Manufacturers String descriptor */
              case DEF_STRING_DESC_MANU:
                pUSBFS_Descr = descriptor_config_ptr->manufacturer_descriptor;
                len = descriptor_config_ptr->manufacturer_descriptor[0];
                break;

              /* Descriptor 2, Product String descriptor */
              case DEF_STRING_DESC_PROD:
                pUSBFS_Descr = descriptor_config_ptr->prod_info_descriptor;
                len          = descriptor_config_ptr->prod_info_descriptor[0];
                break;

              /* Descriptor 3, Serial-number String descriptor */
              case DEF_STRING_DESC_SERN:
                pUSBFS_Descr =
                    descriptor_config_ptr->serial_num_info_descriptor;
                len = descriptor_config_ptr->serial_num_info_descriptor[0];
                break;
          ...
          case USB_CLEAR_FEATURE:
            if ((USBFS_SetupReqType & USB_REQ_RECIP_MASK) ==
                USB_REQ_RECIP_DEVICE) {
              /* clear one device feature */
              if ((uint8_t)(USBFS_SetupReqValue & 0xFF) ==
                  USB_REQ_FEAT_REMOTE_WAKEUP) {
                /* clear usb sleep status, device not prepare to sleep */
                USBFS_DevSleepStatus &= ~0x01;
              }
            } else if ((USBFS_SetupReqType & USB_REQ_RECIP_MASK) ==
                       USB_REQ_RECIP_ENDP) {
              /* Clear End-point Feature */
              if ((uint8_t)(USBFS_SetupReqValue & 0xFF) ==
                  USB_REQ_FEAT_ENDP_HALT) {
          ...
          case USB_SET_FEATURE:
            if ((USBFS_SetupReqType & USB_REQ_RECIP_MASK) ==
              ...
              if ((uint8_t)(USBFS_SetupReqValue & 0xFF) ==
                  USB_REQ_FEAT_REMOTE_WAKEUP) {
                if (descriptor_config_ptr->config_descriptor[7] & 0x20) {
          ...
          /* This request allows the host to select another setting for the
           * specified interface  */
          ...
          case USB_GET_STATUS:
            ...
            if ((USBFS_SetupReqType & USB_REQ_RECIP_MASK) ==
              ...
              case (DEF_UEP_IN | DEF_UEP3):
                if (((USBFSD->UEP3_CTRL_H) & USBFS_UEP_T_RES_MASK) ==
                    USBFS_UEP_T_RES_STALL) {
                  USBFS_EP0_4Buf[0] = 0x01;
                }
                break;
        ...
        /* errflag = 0xFF means a request not support or some errors occurred,
         * else correct */
        if (errflag == 0xff) {
      ...
      case USBFS_UIS_TOKEN_SETUP:
        ...
        if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) != USB_REQ_TYP_STANDARD) {
          if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS) {
            ...
            case HID_GET_IDLE:
              if (USBFS_SetupReqIndex == 0x00) {
                USBFS_EP0_4Buf[0] = USBFS_HidIdle[0];
                len               = 1;
              } else if (USBFS_SetupReqIndex == 0x01) {
                USBFS_EP0_4Buf[0] = USBFS_HidIdle[1];
                len               = 1;
              } else {
                errflag = 0xFF;
              }
            ...
            case HID_GET_PROTOCOL:
              if (USBFS_SetupReqIndex == 0x00) {
                USBFS_EP0_4Buf[0] = USBFS_HidProtocol[0];
                len               = 1;
              } else if (USBFS_SetupReqIndex == 0x01) {
                USBFS_EP0_4Buf[0] = USBFS_HidProtocol[1];
                len               = 1;
              } else {
                errflag = 0xFF;
              }
          ...
          case USB_GET_DESCRIPTOR:
            ...
            case USB_DESCR_TYP_HID:
              if (USBFS_SetupReqIndex == 0x00) {
                pUSBFS_Descr = &descriptor_config_ptr->config_descriptor[18];
                len          = 9;
              }
              // else if (USBFS_SetupReqIndex == 0x01) {
              //   pUSBFS_Descr = &descriptor_config_ptr->config_descriptor[43];
              //   len          = 9;
              // }
              else {
                errflag = 0xFF;
            ...
            case USB_DESCR_TYP_REPORT:
              if (USBFS_SetupReqIndex == 0x00) {
                pUSBFS_Descr =
                    descriptor_config_ptr->consumer_report_descriptor;
                len = 0x27;
              } else {
                errflag = 0xFF;
              }
            ...
            if ((USBFS_SetupReqType & USB_REQ_RECIP_MASK) ==
              ...
              if (USBFS_DevSleepStatus & 0x01) {
                ...
                USBFS_EP0_4Buf[0] = 0x00;
                       ...
                       USB_REQ_RECIP_ENDP) {
              if ((uint8_t)(USBFS_SetupReqIndex & 0xFF) ==
                  (DEF_UEP_IN | DEF_UEP1)) {
                if ((USBFSD->UEP1_CTRL_H & USBFS_UEP_T_RES_MASK) ==
                    USBFS_UEP_T_RES_STALL) {
                  USBFS_EP0_4Buf[0] = 0x01;
                }
              } else if ((uint8_t)(USBFS_SetupReqIndex & 0xFF) ==
                         (DEF_UEP_IN | DEF_UEP2)) {
...
bool is_endpoint_rx_pending(uint8_t endpoint) {
  if (endpoint <= DEF_UEP_NUM - 1) // Endpoint index starts at 0
  {
    return endpoint_rx_pending[endpoint];
  }
  return false;
}
...
uint8_t get_endpoint_rx_message(uint8_t endpoint, uint8_t *buffer,
  ...
  /* Alert the system there is data waiting on endpoint */
  size_t   len = USBFSD->RX_LEN;
  uint8_t *endpoint_buffer_ptr;
  switch (endpoint) {
  case 2:
    endpoint_buffer_ptr = USBFS_EP2_Buf;
    break;
  default:
    break;
  }
  if (len <= buffer_length) {
    memcpy(buffer, endpoint_buffer_ptr, len);
  }
  endpoint_rx_pending[endpoint] = false;
  return len;
}

bool is_endpoint_tx_pending(uint8_t endpoint) {
  return USBFS_Endp_Busy[endpoint];
}