...
#define JVC_NEW_MESSAGE_DELAY_MS          50
...
void JVC_SWC::jvc_output_swc(uint8_t swc_command) {
  uint8_t message_repitions = (this->_previous_command == swc_command)
                                  ? JVC_REPEATED_MESSAGE_REPITITIONS
                                  : JVC_NEW_MESSAGE_REPITITIONS;
  ...
  while (message_repitions) {
    ...
    message_repitions--;
    delay(JVC_MESSAGE_REPEAT_DELAY_MS);
  }
  delay(JVC_NEW_MESSAGE_DELAY_MS);
}
...
/* Write a binary one. We are controlling an open-drain output so we must invert
 * the signals */
